...
column_ref_d = 2*1670/500*np.infty
t_tot = 1./bovy_conversion.time_in_Gyr(220.,8.); n_t = 900.
ts= np.linspace(0,t_tot,n_t) 
sigmav = 8/np.sqrt(3)/220.
sigma_d0 = 0.29*8.*bovy_conversion.force_in_kmsMyr(220.,8.)/pow(220.,2)*1000*3.086/3.154
...
def Coll(kind,R,phi,z,vR,vT,vz,r):
    ...
    if (kind==1):
        sigma_d = np.sqrt(-nfw.__call__(R,z)*pow(1+nfw.__call__(R,z)/2.4261154,0.41)*sigma_d0)
        veld = np.array([np.random.normal(0,sigma_d), np.random.normal(0,sigma_d), np.random.normal(0,sigma_d)])
        vel = 0.5*veld+0.5*veldm-0.5*np.linalg.norm(veldm-veld)*np.array([np.sin(theta)*np.cos(phi),np.sin(theta)*np.sin(phi),np.cos(theta)])
    return (vel)
...
def particle(num):
    ...
    column_d=0.; column0_d = -column_ref_d*np.log(random.random())
    time = 0.; tpick = 0.
    numcoll_b = 0; numcoll_d = 0; i_tf = 0
    ...
    cst_b_use = cst_b*10.5/8.; cst_Ghalo_use = cst_Ghalo*10.5/8.; cst_d_use = cst_d*10.5/8.   
    ...
    while (time<10.5):
        ...
        for t in ts_use:
            ...
            column_d+=cst_d_use/o.r(t)/pow(1+o.r(t)/2.,2)*np.linalg.norm(np.array([o.vR(t),o.vT(t),o.vz(t)])) 
            if (column_b>column0_b and column_d>column0_d):
                #lock.acquire()
                ...
                numcoll_d+=1; column_d = 0.; column0_d = -column_ref_d*np.log(random.random())
                if (random.random()>0.5):
                    vel=Coll(0,o.R(t),o.phi(t),o.z(t),o.vR(t),o.vT(t),o.vz(t),o.r(t))
                    vel=Coll(1,o.R(t),o.phi(t),o.z(t),vel[0],vel[1],vel[2],o.r(t))
                else:
                    vel=Coll(1,o.R(t),o.phi(t),o.z(t),o.vR(t),o.vT(t),o.vz(t),o.r(t))
                    vel=Coll(0,o.R(t),o.phi(t),o.z(t),vel[0],vel[1],vel[2],o.r(t))
                flag=False
                    
            elif (column_b > column0_b):
                numcoll_b+=1; column_b = 0.; column0_b = -column_ref_b*np.log(random.random())  
                vel=Coll(0,o.R(t),o.phi(t),o.z(t),o.vR(t),o.vT(t),o.vz(t),o.r(t))#kind-0 for baryon
                flag=False
                
            elif (column_d > column0_d):
                numcoll_d+=1; column_d = 0.; column0_d = -column_ref_d*np.log(random.random())
                vel=Coll(1,o.R(t),o.phi(t),o.z(t),o.vR(t),o.vT(t),o.vz(t),o.r(t))#kind-1 for DM 
                flag=False
            ...
            if(flag==False):
                ...
                flag=True   
                break
        ts_use=ts; cst_b_use= cst_b; cst_Ghalo_use= cst_Ghalo; cst_d_use=cst_d
        if (numcoll_b>30 or numcoll_d>40):
